...
void setup()
{
  ...
  Tft.drawString("Smart Alarm", MAX_X-70, 10, 3, GREEN);
...
void loop()
{
  ...
  while(tf==true){
    ...
    Tft.drawString("The Idea Works", 50, 50, 3, CLOCK_COLOR);
    if(p.z > ts.pressureThreshhold){
      tf=false;
      Tft.paintScreenBlack();
    }
...
void draw_display(int time, int alarmTime) {
 ...
 // Tft.fillRectangle(MAX_X-65, 0, 65, MAX_Y, RED);
 // Tft.fillRectangle(0, 0, BUTTON_HEIGHT, BUTTON_WIDTH, RED);
 // Tft.fillRectangle(BUTTON_HEIGHT+20, 0, MAX_X-BUTTON_HEIGHT-105, MAX_Y, BLUE);
  Tft.drawString("Weather Condition", BUTTON_HEIGHT+50, 100, 1, CLOCK_COLOR);
  Tft.drawString("Temp", BUTTON_HEIGHT+ 80, 80, 2, CLOCK_COLOR);
  Tft.drawString("Hi|Lo", BUTTON_HEIGHT + 80, 170, 2, CLOCK_COLOR);
  Tft.drawString(timeString, 190, 25, 7, CLOCK_COLOR);
  Tft.drawString("Date Goes Here", MAX_X - 10, 50, 2, CLOCK_COLOR);
  Tft.drawString("Set", 30, 30, 2, WHITE);
  int hour12, minute;
  bool am;
  time_to_hour_minute(time, hour12, minute, am);
  // Draw alarm time.
  if (alarmTime >= 0) {
    ...
    Tft.drawString(timeString, MAX_X-70, 200, 3, CYAN);
 ...
 // Tft.fillRectangle(0, 210, BUTTON_HEIGHT, BUTTON_WIDTH, CLOCK_COLOR);
  Tft.drawString("Refrsh", 30, REFRESH_Y+5, 2, WHITE);
}
